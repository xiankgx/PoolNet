...
class Solver(object):
            ...
            if test_mode == 0:
                ...
                for k in range(0, len(scale)):
                    ...
                    with torch.no_grad():
                        ...
                        pred_0 = np.squeeze(torch.sigmoid(
                            preds[1][0]).cpu().data.numpy())
                        pred_1 = np.squeeze(torch.sigmoid(
                            preds[1][1]).cpu().data.numpy())
                        pred_2 = np.squeeze(torch.sigmoid(
                            preds[1][2]).cpu().data.numpy())
                        pred_fuse = np.squeeze(
                            torch.sigmoid(preds[0]).cpu().data.numpy())
    ...
    # training phase
    def train(self):
        # each batch only train 30000 iters.(This number is just a random choice...)
        iter_num = 30000
        aveGrad = 0
        for epoch in range(self.config.epoch):
            ...
            if (epoch + 1) % self.config.epoch_save == 0:
                torch.save(self.net.state_dict(), '%s/models/epoch_%d.pth' %
