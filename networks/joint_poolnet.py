...
class DeepPoolLayer(nn.Module):
    ...
    def forward(self, x, x2=None, x3=None):
        ...
        for i in range(len(self.pools_sizes)):
            ...
            resl = torch.add(resl, F.interpolate(y, x_size[2:],
                                                 mode='bilinear',
                                                 align_corners=True))
        resl = self.relu(resl)

        if self.need_x2:
            resl = F.interpolate(resl, x2.size()[2:],
                                 mode='bilinear',
                                 align_corners=True)
...
class BlockLayer(nn.Module):
    def __init__(self, k_in, k_out_list):
        ...
        for k in k_out_list:
            ...
            up_mid1.append(
                nn.Sequential(
                    nn.Conv2d(k_in//4, k_in//4, 3, 1, 1, bias=False),
                    nn.Conv2d(k_in//4, k_in, 1, 1, bias=False)
                )
            )
...
class EdgeInfoLayerC(nn.Module):
    def __init__(self, k_in, k_out):
        ...
        self.trans = nn.Sequential(
            nn.Conv2d(k_in, k_in, 3, 1, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(k_in, k_out, 3, 1, 1, bias=False),
            nn.ReLU(inplace=True),
...
class FuseLayer1(nn.Module):
    ...
    def forward(self, list_x, x_size):
        ...
        for i, i_x in enumerate(list_x):
            up_x.append(F.interpolate(self.trans[i](i_x), x_size[2:],
                                      mode='bilinear',
                                      align_corners=True))
...
def extra_layer(base_model_cfg, base):
    ...
    for i in range(len(config['deep_pool'][0])):
        deep_pool_layers += [DeepPoolLayer(config['deep_pool'][0][i],
                                           config['deep_pool'][1][i],
                                           config['deep_pool'][2][i],
                                           config['deep_pool'][3][i])]
