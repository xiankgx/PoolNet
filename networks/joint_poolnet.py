...
config_vgg = {'convert': [[128, 256, 512, 512, 512], [64, 128, 256, 512, 512]], 'deep_pool': [[512, 512, 256, 128], [512, 256, 128, 128], [True, True, True, False], [
    True, True, True, False]], 'score': 256, 'edgeinfoc': [48, 128], 'block': [[512, [16]], [256, [16]], [128, [16]]], 'fuse': [[16, 16, 16], True]}  # no convert layer, no conv6

config_resnet = {'convert': [[64, 256, 512, 1024, 2048], [128, 256, 256, 512, 512]], 'deep_pool': [[512, 512, 256, 256, 128], [512, 256, 256, 128, 128], [False, True, True, True, False], [
    True, True, True, True, False]], 'score': 256, 'edgeinfoc': [64, 128], 'block': [[512, [16]], [256, [16]], [256, [16]], [128, [16]]], 'fuse': [[16, 16, 16, 16], True]}
...
class ConvertLayer(nn.Module):
    def __init__(self, list_k):
        ...
        for i in range(len(list_k[0])):
            up.append(nn.Sequential(nn.Conv2d(
                list_k[0][i], list_k[1][i], 1, 1, bias=False), nn.ReLU(inplace=True)))
...
class DeepPoolLayer(nn.Module):
    ...
    def forward(self, x, x2=None, x3=None):
        ...
        for i in range(len(self.pools_sizes)):
            ...
            resl = torch.add(resl, F.interpolate(
                y, x_size[2:], mode='bilinear', align_corners=True))
        resl = self.relu(resl)
        if self.need_x2:
            resl = F.interpolate(
                resl, x2.size()[2:], mode='bilinear', align_corners=True)
...
class BlockLayer(nn.Module):
    def __init__(self, k_in, k_out_list):
        ...
        for k in k_out_list:
            ...
            up_mid1.append(nn.Sequential(nn.Conv2d(
                k_in//4, k_in//4, 3, 1, 1, bias=False), nn.Conv2d(k_in//4, k_in, 1, 1, bias=False)))
            up_in2.append(nn.Conv2d(k_in, k_in//4, 1, 1, bias=False))
            up_mid2.append(nn.Sequential(nn.Conv2d(
                k_in//4, k_in//4, 3, 1, 1, bias=False), nn.Conv2d(k_in//4, k_in, 1, 1, bias=False)))
    ...
    def forward(self, x, mode=0):
        ...
        x_tmp = self.relu(
            x_tmp + self.block_mid2[mode](self.block_in2[mode](x_tmp)))
...
class EdgeInfoLayerC(nn.Module):
    def __init__(self, k_in, k_out):
        ...
        self.trans = nn.Sequential(nn.Conv2d(k_in, k_in, 3, 1, 1, bias=False), nn.ReLU(inplace=True),
                                   nn.Conv2d(k_in, k_out, 3, 1, 1, bias=False), nn.ReLU(
                                       inplace=True),
                                   nn.Conv2d(k_out, k_out, 3, 1, 1, bias=False), nn.ReLU(
                                       inplace=True),
    ...
    def forward(self, x, x_size):
        tmp_x = []
        for i_x in x:
            tmp_x.append(F.interpolate(
                i_x, x_size[2:], mode='bilinear', align_corners=True))
...
class FuseLayer1(nn.Module):
    ...
    def forward(self, list_x, x_size):
        ...
        for i, i_x in enumerate(list_x):
            up_x.append(F.interpolate(self.trans[i](
                i_x), x_size[2:], mode='bilinear', align_corners=True))
...
class ScoreLayer(nn.Module):
    ...
    def forward(self, x, x_size=None):
        ...
        if x_size is not None:
            x = F.interpolate(
                x, x_size[2:], mode='bilinear', align_corners=True)
        return x
...
def extra_layer(base_model_cfg, base):
    ...
    for i in range(len(config['deep_pool'][0])):
        deep_pool_layers += [DeepPoolLayer(config['deep_pool'][0][i], config['deep_pool']
                                           [1][i], config['deep_pool'][2][i], config['deep_pool'][3][i])]

    fuse_layers = FuseLayer1(config['fuse'][0], config['fuse'][1])
    edgeinfo_layers = EdgeInfoLayerC(
        config['edgeinfoc'][0], config['edgeinfoc'][1])
...
class PoolNet(nn.Module):
    ...
    def forward(self, x, mode):
        ...
        if mode == 0:
            ...
            edge_merge = [self.block[i](kk).detach()
                          for i, kk in enumerate(edge_merge)]
